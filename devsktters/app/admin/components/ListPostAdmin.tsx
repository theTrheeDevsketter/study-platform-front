...
export default function ListPostAdmin() {
   ...
   const loadPost = async () =>{
      const postsFromHell = await getData() as Posti[]
      
      setPosts(postsFromHell)
   }
  ...
  return (
   ...
   <>
      ...
      {
      posts?.map(({author,title,tags,avatarImg,id,date,likes,content,isPosted},i) =>
        !isPosted && <Link key={author + i.toString()} href={`/admin/${id}`} >
